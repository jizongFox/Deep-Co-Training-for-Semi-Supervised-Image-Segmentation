...
def get_ACDC_dataloaders(dataset_dict: dict, dataloader_dict: dict, quite=False, mode1='train',mode2='val'):
    ...
    train_set = MedicalImageDataset(mode=mode1, **dataset_dict, quite=quite)
    val_set = MedicalImageDataset(mode=mode2, **dataset_dict, quite=quite)
...
def extract_patients_gmc(dataloader: DataLoader, patient_ids: List[str], site_id: List[int] = 1):
    assert isinstance(patient_ids, list)
    # bpattern = lambda d: 'patient%.3d' % int(d)
    bpattern = lambda site, pait, img: 'site{}-sc{:02d}-image_{}'.format(int(site), int(pait), int(img))
    patterns = re.compile('|'.join(
        [bpattern(site_id, n_pat+1, n_img+1) for n_pat in range(1, 11) for n_img in range(3)])
    )
