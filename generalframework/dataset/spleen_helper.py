...
from typing import List, Callable, Pattern, Iterable, TypeVar, Match, Dict
...
def get_spleen_dataloaders(dataset_dict: dict, dataloader_dict: dict, quite=False, mode1='train', mode2='val'):
    dataset_dict['root_dir'] = Path(__file__).parents[2] / 'dataset' / 'Spleen'
...
def get_spleen_split_dataloders(config):
    ...
    dataloders = get_spleen_dataloaders(config['Dataset'], config['Lab_Dataloader'])
    ...
    unlab_dataloader = get_spleen_dataloaders(config['Dataset'], config['Unlab_Dataloader'], quite=True)['train']
...
def extract_patients(dataloader: DataLoader, patient_ids: List[str]):
    ...
    bpattern = lambda d: 'Patient_%.2d' % int(d)
