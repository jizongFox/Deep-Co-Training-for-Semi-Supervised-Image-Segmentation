...
from generalframework.loss.loss import JSD_2D, Entropy_2D
...
# criterion = JSD_2D(num_probabilities=2)
...
def test_inference():
    ...
    net = get_arch('enet',{'num_classes':2})
    net = net.cuda()
    with torch.no_grad():
        for i in range (1000):
            img = torch.randn(20, 1, 256, 256)
            img = img.cuda()
            pred = net(img)
