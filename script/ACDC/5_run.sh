...
if [ $num_models = 2 ]; then
	...
	bash 5_multiple_views.sh $logdir FS $num_models $max_epoch $gpu &
	bash 5_multiple_views.sh $logdir JSD $num_models $max_epoch $gpu &

	wait_script
	bash 5_multiple_views.sh $logdir PS $num_models $max_epoch $gpu &
	bash 5_multiple_views.sh $logdir JSD_ADV $num_models $max_epoch $gpu &
	wait_script
fi
